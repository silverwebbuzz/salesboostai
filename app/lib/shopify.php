...
/**
 * Register the webhooks SalesBoost uses to stay in sync.
 */
...
/**
 * Ensure a store has a subscription row (default: free).
 */
function ensureFreeSubscription(string $shop): void
{
    ...
    $stmt = $mysqli->prepare(
        "INSERT INTO store_subscription (shop, plan_key, status)
         VALUES (?, ?, ?)
         ON DUPLICATE KEY UPDATE
            shop = VALUES(shop)"
    );
    ...
    $stmt->bind_param('sss', $shop, $planKey, $status);
    $stmt->execute();
    $stmt->close();
}

/**
 * Mark subscription inactive on uninstall.
 */
...
{
    $mysqli = db();
    $stmt = $mysqli->prepare("SELECT * FROM store_subscription WHERE shop = ? LIMIT 1");
    if (!$stmt) {
        return null;
    }
    $stmt->bind_param('s', $shop);
...
/**
 * Upsert current subscription state (single row per shop).
 */
    ...
    $stmt = $mysqli->prepare(
        "INSERT INTO store_subscription (shop, plan_key, status, shopify_charge_id, current_period_ends_at, cancelled_at)
         ...
         ON DUPLICATE KEY UPDATE
            ...
            shopify_charge_id = VALUES(shopify_charge_id),
            current_period_ends_at = VALUES(current_period_ends_at),
            cancelled_at = IF(VALUES(status) IN ('cancelled','free'), NOW(), NULL),
            updated_at = NOW()"
    );
    if (!$stmt) {
        throw new Exception('Prepare failed: ' . $mysqli->error);
    }
    $chargeId = $shopifyChargeId ?? null;
    $cpea = $currentPeriodEndsAt ?? null;
    $stmt->bind_param('sssss', $shop, $planKey, $status, $chargeId, $cpea);
    $stmt->execute();
    $stmt->close();
}
